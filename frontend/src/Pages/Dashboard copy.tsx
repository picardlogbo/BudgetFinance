...
export const Dashboard: React.FC<DashboardProps> = ({ revenus, depenses, factures, savings }) => {
  ...
  const Factures_En_Attente = factures.filter(f => f.status === 'En_attente');
  const Factures_En_Retard = factures.filter(f => f.status === 'En_retard');

  const totalSavings = savings.reduce((sum, s) => sum + s.currentAmount, 0);
  // const savingsGoals = savings.reduce((sum, s) => sum + s.targetAmount, 0);
    ...
    <div className="space-y-6">
              ...
              <div className="flex items-center p-3 bg-red-50 border border-red-200 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-red-600">
                    Total: {Factures_En_Retard.reduce((sum, bill) => sum + bill.montant, 0).toLocaleString('fr-FR')} €
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="flex items-center p-3 bg-orange-50 border border-orange-200 rounded-lg">
                ...
                <div>
                  ...
                  <p className="text-sm text-orange-600">
                    Total: {Factures_En_Attente.reduce((sum, bill) => sum + bill.montant, 0).toLocaleString('fr-FR')} €
                  </p>
                </div>
              </div>
            )}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {['food', 'transport', 'housing', 'entertainment'].map((category) => {
            const categoryExpenses = depenses
              .filter(e => e.categorie === category && new Date(e.createdAt).getMonth() === currentMonth)
              .reduce((sum, e) => sum + e.montant, 0);
