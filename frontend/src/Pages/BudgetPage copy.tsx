...
//   const filteredBudgets = useMemo(() => budgets.filter(b => b.mois === filterMonth && b.annee === filterYear), [budgets, filterMonth, filterYear]);

//   // Agrégation pour total période
//   const totalAllocated = useMemo(() => filteredBudgets.reduce((s, b) => s + b.montantAlloue, 0), [filteredBudgets]);
//   const totalSpent = useMemo(() => filteredBudgets.reduce((s, b) => s + b.montantDepense, 0), [filteredBudgets]);

//   // Dépenses liées au budget sélectionné (dans un vrai flux -> GET /depenses?budget=ID)
//   const selectedBudget = useMemo(() => filteredBudgets.find(b => b.id === selectedBudgetId) || null, [filteredBudgets, selectedBudgetId]);
...
//           return (
//             <button key={b.id} onClick={() => setSelectedBudgetId(b.id)} className={`relative group bg-white border rounded-2xl p-5 flex flex-col items-center gap-4 shadow-sm hover:shadow transition ${selectedBudgetId === b.id ? "ring-2 ring-blue-500" : ""}`}>
